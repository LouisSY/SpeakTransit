...
class Camera: NSObject {
    ...
    func setShutterSpeed(_ duration: CMTime) {
        ...
        do {
            ...
            // set ISO（keep current ISO）
            let iso = device.iso

            // make sure to enable exposure mode manully
            if device.isExposureModeSupported(.custom) {
                device.setExposureModeCustom(duration: duration, iso: iso, completionHandler: nil)
                logger.debug("Shutter speed set to \(duration.seconds) seconds")
            } else {
                logger.error("Custom exposure mode is not supported on this device.")
            }

            device.unlockForConfiguration()
        } catch {
            logger.error("Failed to set shutter speed: \(error.localizedDescription)")
        }
    }
...
//                captureDevice = availableCaptureDevices.first ?? AVCaptureDevice.default(for: .video)
                self.captureDevices.forEach { print($0.localizedName) }
                captureDevice = AVCaptureDevice.default(.builtInWideAngleCamera, for: .video, position: .back)
                print("Device Name: \(captureDevice!.localizedName)")
                print("Position: \(captureDevice!.position == .back ? "Back" : "Front")")
                print("Exposure Mode Supported: \(captureDevice!.isExposureModeSupported(.custom) ? "Yes" : "No")")
                print("ISO Range: \(captureDevice!.activeFormat.minISO) - \(captureDevice!.activeFormat.maxISO)")
                print("Exposure Duration Range: \(captureDevice!.activeFormat.minExposureDuration.seconds) - \(captureDevice!.activeFormat.maxExposureDuration.seconds) sec")
